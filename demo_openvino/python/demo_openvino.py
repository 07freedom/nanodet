...
"""
name:       demo_openvino.py
date:       2021-10-09 11:21:07
Env.:       Python 3.6.13, Ubuntu
"""
...
class NanoDetOPENVINO(NanoDetABC):
    from openvino.inference_engine import IECore, IENetwork

    def __init__(self, model_path, *args, **kwargs):
        # from openvino.inference_engine import IECore, IENetwork
        super(NanoDetOPENVINO, self).__init__(*args, **kwargs)
        print("Using OPENVINO as inference backend")
        ...
        self.net = self.IENetwork(
            model=self.model_path, weights=self.model_path.replace(".xml", ".bin")
        )
        self.exec_net = ie.load_network(
            network=self.net, num_requests=1, device_name=DEVICE
        )
        self.input_blob = next(iter(self.net.inputs))

    def infer_image(self, img_input):
        res = self.exec_net.infer(inputs={self.input_blob: img_input})
        # print(res)
        score_out_name = [
            "cls_pred_stride_8",
            "cls_pred_stride_16",
        ...
        scores = [res[x] for x in score_out_name]
        ...
        raw_boxes = [res[x] for x in boxes_out_name]
...
def main():
    ...
    parser.add_argument(
        "--model_path", dest="model_path", type=str, default="../model/nanodet-320.bin"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--backend", choices=["OPENVINO", "ONNX", "torch"], default="OPENVINO"
    )
    ...
    if args.backend == "OPENVINO":
        detector = NanoDetOPENVINO(args.model_path, input_shape=args.input_shape)
...
def test_one():
    detector = NanoDetOPENVINO("../nanodet_m_1.5x_coco.xml")
